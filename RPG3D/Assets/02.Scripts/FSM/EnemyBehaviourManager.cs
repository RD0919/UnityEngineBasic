        ...
        _behaviourTree.StartBuild(gameObject)
            .Selector()
                .parallel(Parallel.Policy.RequireOne)
                    .Condition(() => _behaviourTree.blackBoard.agent.remainingDistance <= _behaviourTree.blackBoard.agent.stoppingDistance)
                        .Patrol(_patrol_range)
                    .Seek(_seek_radius, _seek_angle, _seek_deltaAngle, _seek_targetMask, _seek_offset)
                    .Condition(() => _behaviourTree.blackBoard.target != null && Vector3.Distance(_behaviourTree.blackBoard.target.position, transform.position) < _attack_range)
